...
func GetPlugin(config common.Config) (*model.Plugin, error) {
	datasourceClient := &model.DatasourceClient{
		DataClient: datasource.GetDataClient(datasource.PluginLocationExternal, &sql.DataClient{Config: config}, config),
	}

	return model.MakePlugin(model.PluginTypeDatasource, "altibase", "Altibase", jsonSchema, uiSchema, datasourceClient, sampleFormData)
}
