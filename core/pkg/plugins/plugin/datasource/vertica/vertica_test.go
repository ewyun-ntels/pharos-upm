...
func TestGetPlugin(t *testing.T) {
	...
	if plugin.ID != "vertica" {
		t.Errorf("Expected plugin ID 'vertica', got '%s'", plugin.ID)
	}

	if plugin.Name != "Vertica" {
		t.Errorf("Expected plugin name 'Vertica', got '%s'", plugin.Name)
	}
...
func TestEmbeddedJSONSchema(t *testing.T) {
	...
	if typeProperty["default"] != "vertica" {
		t.Errorf("Expected type default 'vertica', got %v", typeProperty["default"])
	}
	...
	if portProperty["default"] != float64(5433) {
		t.Errorf("Expected port default 5433, got %v", portProperty["default"])
	}
...
func TestEmbeddedSampleFormData(t *testing.T) {
	...
	if data["type"] != "vertica" {
		t.Errorf("Expected type 'vertica', got %v", data["type"])
	}

	if data["name"] != "sample-vertica" {
		t.Errorf("Expected name 'sample-vertica', got %v", data["name"])
	}
	...
	if dataField["host"] != "192.168.15.101" {
		t.Errorf("Expected host '192.168.15.101', got %v", dataField["host"])
	}
	...
	if port != 30000 {
		t.Errorf("Expected port 30000, got %v", port)
	}

	if dataField["database"] != "vertdb" {
		t.Errorf("Expected database 'vertdb', got %v", dataField["database"])
	}

	if dataField["username"] != "dbadmin" {
		t.Errorf("Expected username 'dbadmin', got %v", dataField["username"])
	}

	if dataField["password"] != "dbadmin" {
		t.Errorf("Expected password 'dbadmin', got %v", dataField["password"])
	}
...
func TestVerticaConstants(t *testing.T) {
	expectedType := "vertica"
...
func TestVerticaSchemaValidation(t *testing.T) {
	// Test that the JSON schema contains Vertica-specific properties
	var schema map[string]any
	err := json.Unmarshal([]byte(jsonSchema), &schema)
	if err != nil {
		t.Fatalf("JSON schema is not valid JSON: %v", err)
	...
	// Check for Vertica-specific required fields
...
func TestGetPlugin_WithDifferentConfigs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if plugin.ID != "vertica" {
				t.Errorf("GetPlugin() plugin ID = %v, want %v", plugin.ID, "vertica")
			}
		})
	}
}
