...
// 현재 테이블의 필드들이 nullable로 정의되어 있음
type postgresUser struct {
	Username          string     `db:"username"`            // text not null
	PasswordHash      []byte     `db:"password_hash"`       // text
	Extra             []byte     `db:"extra"`               // text
	CreatedAt         *time.Time `db:"created_at"`          // timestamp
	Retry             *int       `db:"retry"`               // real
	RetryAt           *time.Time `db:"retry_at"`            // timestamp
	Blocked           *bool      `db:"blocked"`             // boolean
	Prepare           []byte     `db:"prepare"`             // text
	PasswordExpiredAt *time.Time `db:"password_expired_at"` // timestamp
	PasswordUpdatedAt *time.Time `db:"password_updated_at"` // timestamp
}

func (repo *userRepository) toUser(dbUser postgresUser) (*repositories.UserEntity, error) {
	...
	if dbUser.Retry != nil {
		u.Retry = *dbUser.Retry
	}

	if dbUser.Blocked != nil {
...
func (repo *userRepository) toDBTime(t time.Time) time.Time {
	// postgres는 시간을 timestamp without time zone으로 저장하는데 UTC가 아닌 경우
	// 정확한 시간을 알 수 없기 때문에 UTC로 변환하여 저장
	return t.UTC()
}

func (repo *userRepository) toNullableTime(t time.Time) *time.Time {
	if !t.IsZero() {
		dbTime := repo.toDBTime(t)
		return &dbTime
	}
	return nil
}
...
func (repo *userRepository) Create(ctx context.Context, u *repositories.UserEntity, password []byte) error {
	query := `
INSERT INTO users
    (username, password_hash, extra, created_at, retry, retry_at, blocked, password_expired_at, prepare)
VALUES ($1, $2, $3, (NOW() AT TIME ZONE 'UTC'), 0, null, false, $4, $5)`
	...
	passwordExpiredAt := repo.toNullableTime(u.PasswordExpiredAt)
...
UPDATE users SET
    password_hash = $1,
    password_updated_at = (NOW() AT TIME ZONE 'UTC'),
...
WHERE username = $3`
	...
	expiredAt := repo.toNullableTime(passwordExpiredAt)
...
WHERE username = $2`
	...
	t := repo.toNullableTime(expiredAt)
