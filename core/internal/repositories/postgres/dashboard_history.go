...
type postgresDashboardHistory struct {
	ID             string    `db:"id"`
...
func (r *postgresDashboardHistory) toEntity() repositories.DashboardHistoryRecord {
...
func (repo *dashboardHistoryRepository) Query(ctx context.Context, limit, offset int, search string) ([]repositories.DashboardHistoryRecord, error) {
	var rows []postgresDashboardHistory
	var err error
	if search != "" {
		like := "%" + search + "%"
		q := `SELECT id, dashboard_id, dashboard_title, action, changed_by, changed_at, config_json
			FROM dashboard_history
			WHERE dashboard_title ILIKE $1 OR action ILIKE $2 OR changed_by ILIKE $3
			ORDER BY changed_at DESC LIMIT $4 OFFSET $5`
		err = repo.Handler(func(db *sqlx.DB) error {
			return db.SelectContext(ctx, &rows, q, like, like, like, limit, offset)
		})
	} else {
		q := `SELECT id, dashboard_id, dashboard_title, action, changed_by, changed_at, config_json
			FROM dashboard_history ORDER BY changed_at DESC LIMIT $1 OFFSET $2`
		err = repo.Handler(func(db *sqlx.DB) error {
...
func (repo *dashboardHistoryRepository) Count(ctx context.Context, search string) (int, error) {
	...
	if search != "" {
		like := "%" + search + "%"
		q := `SELECT COUNT(*) FROM dashboard_history WHERE dashboard_title ILIKE $1 OR action ILIKE $2 OR changed_by ILIKE $3`
		err = repo.Handler(func(db *sqlx.DB) error {
...
func (repo *dashboardHistoryRepository) Prune(ctx context.Context, maxKeep int) error {
	query := `DELETE FROM dashboard_history WHERE id NOT IN (
		SELECT id FROM dashboard_history ORDER BY changed_at DESC LIMIT $1
	)`
	return repo.Handler(func(db *sqlx.DB) error {
...
func (repo *dashboardHistoryRepository) QueryByDashboardIDs(ctx context.Context, ids []string, limit, offset int, search string) ([]repositories.DashboardHistoryRecord, error) {
	...
	if search != "" {
		...
		q, args, err = sqlx.In(
			...
			FROM dashboard_history
			WHERE dashboard_id IN (?) AND (dashboard_title ILIKE ? OR action ILIKE ? OR changed_by ILIKE ?)
...
func (repo *dashboardHistoryRepository) CountByDashboardIDs(ctx context.Context, ids []string, search string) (int, error) {
	...
	if search != "" {
		like := "%" + search + "%"
		q, args, err = sqlx.In(
			`SELECT COUNT(*) FROM dashboard_history WHERE dashboard_id IN (?) AND (dashboard_title ILIKE ? OR action ILIKE ? OR changed_by ILIKE ?)`,
			ids, like, like, like,
		)
	} else {
	...
	var count int
	err = repo.Handler(func(db *sqlx.DB) error {
		return db.GetContext(ctx, &count, db.Rebind(q), args...)
	})
	return count, err
}
...
func (repo *dashboardHistoryRepository) DeleteByDashboardID(ctx context.Context, dashboardID string) error {
	return repo.Handler(func(db *sqlx.DB) error {
		_, err := db.ExecContext(ctx, `DELETE FROM dashboard_history WHERE dashboard_id = $1`, dashboardID)
		return err
	})
}
