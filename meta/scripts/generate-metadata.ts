...
function generateSiteConfig(projectRoot: string, meta: SiteModeMeta) {
  ...
  const dashboardHomeEnabled = !homeExtension;
  ...
  fs.writeFileSync(
    ...
    `/**\n * Site Config (AUTO-GENERATED)\n * Generated by: pnpm generate:metadata\n */\n\nexport const HOME_PATH = ${JSON.stringify(homePath)};\nexport const DASHBOARD_HOME_ENABLED = ${JSON.stringify(dashboardHomeEnabled)};\n`,
  );
  console.log(`✓ Generated site-config.ts (HOME_PATH: ${homePath}, DASHBOARD_HOME_ENABLED: ${dashboardHomeEnabled})`);
}
