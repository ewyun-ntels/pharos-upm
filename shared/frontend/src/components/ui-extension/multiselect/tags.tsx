...
import {Command, CommandItem, CommandEmpty, CommandList} from '@pharos/shared/components/ui';
import {cn} from '../../../lib';
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent<HTMLDivElement>) => {
      ...
      if ((e.key === 'Backspace' || e.key === 'Delete') && value.length > 0) {
        ...
        if (inputCustomTag) {
          // !value.includes(inputValue) && value.push(inputValue)
          !value.includes(inputValue) && onValueChange([...value, inputValue]);
          setInputValue('');
        }
...
type MultiSelectorInputProps = {
  customInputTag?: boolean;
} & React.ComponentPropsWithoutRef<typeof CommandPrimitive.Input>;

const MultiSelectorInput = forwardRef<
  React.ElementRef<typeof CommandPrimitive.Input>,
  MultiSelectorInputProps
>(({className, customInputTag = false, ...props}, ref) => {
  ...
  return (
    <CommandPrimitive.Input
      ...
      ref={ref}
      value={inputValue}
      // onKeyDown={(e) => {customInputTag && inputHandler(e)}}
