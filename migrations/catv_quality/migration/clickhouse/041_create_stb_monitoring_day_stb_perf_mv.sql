...
-- ========================================
-- mv_stb_monitoring_day_stb_perf 생성
-- stb_monitoring에서 직접 일별 집계
-- v2 Distributed 테이블로 INSERT하여 샤드 2개에 데이터 분산
-- 주의: 이 MV를 활성화하면 신규 데이터는 v2로 수집됩니다
-- ========================================
CREATE MATERIALIZED VIEW IF NOT EXISTS catv.mv_stb_monitoring_day_stb_perf ON CLUSTER clickhouse_cluster
TO catv.dist_stb_monitoring_day_stb_perf
AS SELECT
...
GROUP BY mntr_day, CM_MAC_ADDR, STB_MDL_NM, TPO_CD, NET_CL_CD, INFO_CNTR_CD, HPY_CNTR_ORG_ID, SCRBR_SO_NM, L3_EQUIP_ID, L2_EQUIP_ID, STB_VER_CTT, CCELL_NUM;
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP VIEW IF EXISTS catv.mv_stb_monitoring_day_stb_perf ON CLUSTER clickhouse_cluster SYNC;
-- +goose StatementEnd
