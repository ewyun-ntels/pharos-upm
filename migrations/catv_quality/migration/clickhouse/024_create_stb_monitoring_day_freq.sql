...
-- ========================================
-- stb_monitoring_day_freq 테이블 생성
-- Cell + 주파수 단위 1일 집계를 위한 SummingMergeTree 테이블
-- 목적: 일별 Cell별, 주파수별 품질 분석
-- ========================================
CREATE TABLE IF NOT EXISTS catv.stb_monitoring_day_freq ON CLUSTER clickhouse_cluster_replicated
(
    `mntr_day` Date,
...
ENGINE = ReplicatedSummingMergeTree('/clickhouse/tables/{shard}/catv/stb_monitoring_day_freq', '{replica}')
PARTITION BY toYYYYMM(mntr_day)
ORDER BY (mntr_day, CM_MAC_ADDR, CHNL_FREQ_VAL)
TTL mntr_day + INTERVAL 3 YEAR
COMMENT '일별 Cell + 주파수별 집계 - Cell별, 주파수별 품질 분석용';
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP TABLE IF EXISTS catv.stb_monitoring_day_freq ON CLUSTER clickhouse_cluster_replicated SYNC;
-- +goose StatementEnd
