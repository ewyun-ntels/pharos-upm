...
-- ========================================
-- 1. stb_monitoring_cell_day 테이블 생성
-- 셀×채널 단위 1일 집계 (장기 보관용)
-- ========================================
CREATE TABLE IF NOT EXISTS catv.stb_monitoring_cell_day ON CLUSTER clickhouse_cluster_replicated
(
    -- 시간 (파티션 키)
    mntr_day Date,
...
ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/{shard}/catv/stb_monitoring_cell_day', '{replica}')
PARTITION BY toYYYYMM(mntr_day)
ORDER BY (mntr_day, TPO_CD, INFO_CNTR_CD, HPY_CNTR_ORG_ID, NET_CL_CD,
...
-- +goose StatementBegin
-- ========================================
-- 2. mv_stb_monitoring_cell_day Materialized View 생성
-- stb_monitoring → stb_monitoring_cell_day (원본에서 직접 집계)
-- ========================================
CREATE MATERIALIZED VIEW IF NOT EXISTS catv.mv_stb_monitoring_cell_day ON CLUSTER clickhouse_cluster
TO catv.stb_monitoring_cell_day
AS
SELECT
    -- 시간: 1일 단위로 truncate
...
-- +goose StatementBegin
-- ========================================
-- 3. dist_stb_monitoring_cell_day Distributed 테이블 생성
-- ========================================
CREATE TABLE IF NOT EXISTS catv.dist_stb_monitoring_cell_day ON CLUSTER clickhouse_cluster
AS catv.stb_monitoring_cell_day
ENGINE = Distributed(
    'clickhouse_cluster_replicated',  -- 클러스터 이름
    'catv',                           -- 데이터베이스
    'stb_monitoring_cell_day',        -- 로컬 테이블
...
-- +goose Down
-- +goose StatementBegin
DROP TABLE IF EXISTS catv.dist_stb_monitoring_cell_day ON CLUSTER clickhouse_cluster SYNC;
-- +goose StatementEnd

-- +goose StatementBegin
DROP TABLE IF EXISTS catv.mv_stb_monitoring_cell_day ON CLUSTER clickhouse_cluster SYNC;
-- +goose StatementEnd

-- +goose StatementBegin
DROP TABLE IF EXISTS catv.stb_monitoring_cell_day ON CLUSTER clickhouse_cluster_replicated SYNC;
-- +goose StatementEnd
