...
-- ========================================
-- stb_monitoring_hourly_stb_freq 테이블 생성
-- STB + 주파수 단위 1시간 집계를 위한 SummingMergeTree 테이블
-- 목적: 단말별 주파수별 품질 분석 (Cell 구분 없음)
-- ========================================
CREATE TABLE IF NOT EXISTS catv.stb_monitoring_hourly_stb_freq ON CLUSTER clickhouse_cluster_replicated
(
    `mntr_hour` DateTime,
...
ENGINE = ReplicatedSummingMergeTree('/clickhouse/tables/{shard}/catv/stb_monitoring_hourly_stb_freq', '{replica}')
PARTITION BY toYYYYMM(mntr_hour)
ORDER BY (mntr_hour, CM_MAC_ADDR, CHNL_FREQ_VAL, STB_MDL_NM, TPO_CD, NET_CL_CD, INFO_CNTR_CD, HPY_CNTR_ORG_ID, SCRBR_SO_NM, L3_EQUIP_ID, L2_EQUIP_ID, STB_VER_CTT, CCELL_NUM)
TTL mntr_hour + INTERVAL 3 YEAR
COMMENT 'STB별 주파수별 시간당 집계 - 단말의 주파수별 품질 분석용';
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP TABLE IF EXISTS catv.stb_monitoring_hourly_stb_freq ON CLUSTER clickhouse_cluster_replicated SYNC;
-- +goose StatementEnd
