...
-- ========================================
-- stb_monitoring_hourly_stb_perf 테이블 생성
-- 샤드 2개 구성으로 성능 향상 (레플리카 없음)
-- 목적: 장기 데이터 조회 성능 개선 (약 2배 빠름)
-- ========================================
CREATE TABLE IF NOT EXISTS catv.stb_monitoring_hourly_stb_perf ON CLUSTER clickhouse_cluster
(
    `mntr_hour` DateTime,
...
COMMENT 'STB별 시간당 집계 v2 (샤드 2개, 주파수 구분 없음) - 고성능 장기 조회용';
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP TABLE IF EXISTS catv.stb_monitoring_hourly_stb_perf ON CLUSTER clickhouse_cluster SYNC;
-- +goose StatementEnd
