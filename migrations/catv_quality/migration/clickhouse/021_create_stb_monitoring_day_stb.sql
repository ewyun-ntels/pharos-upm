...
-- ========================================
-- stb_monitoring_day_stb 테이블 생성
-- STB 단위 1일 집계를 위한 SummingMergeTree 테이블
-- 목적: 일별 전체 품질 추이 조회 (주파수 구분 없음)
-- ========================================
CREATE TABLE IF NOT EXISTS catv.stb_monitoring_day_stb ON CLUSTER clickhouse_cluster_replicated
(
    `mntr_day` Date,
...
ENGINE = ReplicatedSummingMergeTree('/clickhouse/tables/{shard}/catv/stb_monitoring_day_stb', '{replica}')
...
COMMENT 'STB별 일별 집계 (주파수 구분 없음) - 일별 품질 추이 조회용';
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP TABLE IF EXISTS catv.stb_monitoring_day_stb ON CLUSTER clickhouse_cluster_replicated SYNC;
-- +goose StatementEnd
