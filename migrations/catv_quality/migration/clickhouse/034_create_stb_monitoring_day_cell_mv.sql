...
-- ========================================
-- mv_stb_monitoring_day_cell 생성
-- stb_monitoring 테이블에서 Cell별로 1일 집계
-- ========================================
CREATE MATERIALIZED VIEW IF NOT EXISTS catv.mv_stb_monitoring_day_cell ON CLUSTER clickhouse_cluster
TO catv.stb_monitoring_day_cell
AS SELECT
...
GROUP BY mntr_day, CCELL_NUM, STB_MDL_NM, TPO_CD, NET_CL_CD, INFO_CNTR_CD, HPY_CNTR_ORG_ID, SCRBR_SO_NM, L3_EQUIP_ID, L2_EQUIP_ID, STB_VER_CTT;
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP VIEW IF EXISTS catv.mv_stb_monitoring_day_cell ON CLUSTER clickhouse_cluster SYNC;
-- +goose StatementEnd
