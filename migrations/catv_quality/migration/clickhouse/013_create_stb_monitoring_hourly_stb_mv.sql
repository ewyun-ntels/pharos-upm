...
-- ========================================
-- mv_stb_monitoring_hourly_stb 생성
-- stb_monitoring 테이블에서 STB별로 1시간 집계
-- STB_QLTY_GR_CD, CHNL_FREQ_VAL 제거하여 순수 STB별 집계만 수행
-- ========================================
CREATE MATERIALIZED VIEW IF NOT EXISTS catv.mv_stb_monitoring_hourly_stb ON CLUSTER clickhouse_cluster
TO catv.stb_monitoring_hourly_stb
AS SELECT
    ...
    -- SNR QAM: sum과 count 저장 (평균은 조회 시 sum/count로 계산, 0 제외)
...
-- +goose Down
-- +goose StatementBegin
DROP VIEW IF EXISTS catv.mv_stb_monitoring_hourly_stb ON CLUSTER clickhouse_cluster SYNC;
-- +goose StatementEnd
