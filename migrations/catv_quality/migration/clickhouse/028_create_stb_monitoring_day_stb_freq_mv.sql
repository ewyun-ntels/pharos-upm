...
-- ========================================
-- mv_stb_monitoring_day_stb_freq 생성
-- stb_monitoring에서 직접 일별 집계
-- ========================================
CREATE MATERIALIZED VIEW IF NOT EXISTS catv.mv_stb_monitoring_day_stb_freq ON CLUSTER clickhouse_cluster
TO catv.stb_monitoring_day_stb_freq
AS SELECT
...
GROUP BY mntr_day, CM_MAC_ADDR, CHNL_FREQ_VAL, STB_MDL_NM, TPO_CD, NET_CL_CD, INFO_CNTR_CD, HPY_CNTR_ORG_ID, SCRBR_SO_NM, L3_EQUIP_ID, L2_EQUIP_ID, STB_VER_CTT, CCELL_NUM;
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP VIEW IF EXISTS catv.mv_stb_monitoring_day_stb_freq ON CLUSTER clickhouse_cluster SYNC;
-- +goose StatementEnd
