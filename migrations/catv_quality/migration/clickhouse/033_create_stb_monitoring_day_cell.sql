...
-- ========================================
-- stb_monitoring_day_cell 테이블 생성
-- Cell 단위 1일 집계를 위한 SummingMergeTree 테이블
-- 목적: 일별 셀 단위 품질 분석 및 추이 조회 (STB 구분 없음)
-- ========================================
CREATE TABLE IF NOT EXISTS catv.stb_monitoring_day_cell ON CLUSTER clickhouse_cluster_replicated
(
    `mntr_day` Date,
...
ENGINE = ReplicatedSummingMergeTree('/clickhouse/tables/{shard}/catv/stb_monitoring_day_cell', '{replica}')
PARTITION BY toYYYYMM(mntr_day)
ORDER BY (mntr_day, CCELL_NUM, STB_MDL_NM, TPO_CD, NET_CL_CD, INFO_CNTR_CD, HPY_CNTR_ORG_ID, SCRBR_SO_NM, L3_EQUIP_ID, L2_EQUIP_ID, STB_VER_CTT)
TTL mntr_day + INTERVAL 5 YEAR
COMMENT '셀별 일별 집계 (STB 구분 없음) - 일별 품질 분석용';
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP TABLE IF EXISTS catv.stb_monitoring_day_cell ON CLUSTER clickhouse_cluster_replicated SYNC;
-- +goose StatementEnd
