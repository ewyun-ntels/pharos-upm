...
-- ========================================
-- mv_stb_monitoring_hourly_stb_freq 생성
-- stb_monitoring 테이블에서 STB + 주파수별로 1시간 집계
-- STB_QLTY_GR_CD 제거, CHNL_FREQ_VAL 포함
-- ========================================
CREATE MATERIALIZED VIEW IF NOT EXISTS catv.mv_stb_monitoring_hourly_stb_freq ON CLUSTER clickhouse_cluster
TO catv.stb_monitoring_hourly_stb_freq
AS SELECT
...
GROUP BY mntr_hour, CM_MAC_ADDR, CHNL_FREQ_VAL, STB_MDL_NM, TPO_CD, NET_CL_CD, INFO_CNTR_CD, HPY_CNTR_ORG_ID, SCRBR_SO_NM, L3_EQUIP_ID, L2_EQUIP_ID, STB_VER_CTT, CCELL_NUM;
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
DROP VIEW IF EXISTS catv.mv_stb_monitoring_hourly_stb_freq ON CLUSTER clickhouse_cluster SYNC;
-- +goose StatementEnd
