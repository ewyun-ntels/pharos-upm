...
func (g *Goflow) handleJobs(w http.ResponseWriter, _ *http.Request) {
	var msg struct {
...
func (g *Goflow) handleRoot(w http.ResponseWriter, _ *http.Request) {
	jobs := make([]*Job, 0)
	...
	tmpl, err := template.ParseFiles("ui/html/index.html.tmpl")
	if err != nil {
		log.Printf("template parse failed: %v", err)
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
	if err := tmpl.Execute(w, map[string]any{"jobs": jobs}); err != nil {
...
func (g *Goflow) handleJobsPage(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		tmpl, err := template.ParseFiles("ui/html/job.html.tmpl")
		if err != nil {
			log.Printf("template parse failed: %v", err)
			http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
			return
...
func (g *Goflow) handleDiagramsPage(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		tmpl, err := template.ParseFiles("ui/html/diagram.html.tmpl")
		if err != nil {
			log.Printf("template parse failed: %v", err)
			http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
			return
